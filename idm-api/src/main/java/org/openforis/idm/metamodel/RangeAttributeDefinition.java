...
public class RangeAttributeDefinition extends NumericAttributeDefinition {
	...
	public List<FieldDefinition<?>> getFieldDefinitions() {
		...
		switch (type) {
		case INTEGER:
			result.add(new FieldDefinition<Integer>("from", "f", "from", Integer.class));
			result.add(new FieldDefinition<Integer>("to", "t", "to", Integer.class));
			break;
		case REAL:
			result.add(new FieldDefinition<Double>("from", "f", "from", Double.class));
			result.add(new FieldDefinition<Double>("to", "t", "to", Double.class));
			break;
		default:
			throw new UnsupportedOperationException("Unknown type");
		}
